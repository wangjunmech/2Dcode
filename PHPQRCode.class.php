 ...
 */class PHPQRCode{ // class start
    ...
    private function add_logo($tmp_qrcode_file){ 
        ...
        if(file_exists($this->_config['logo'])){            // 创建临时二维码图片对象
            ...
            $new_logo_h = isset($this->_config['logo_size'])? $this->_config['logo_size'] : (int)($qrcode_h/5);            // 按设定尺寸调整logo图片
        ...
        $dest_img = imagecreatefrompng($tmp_qrcode_file);            // 生成图片
            switch($dest_ext){
                case 1: imagegif($dest_img, $this->_config['dest_file'], $this->_config['quality']); break;
                case 2: imagejpeg($dest_img, $this->_config['dest_file'], $this->_config['quality']); break;
                case 3: imagepng($dest_img, $this->_config['dest_file'], (int)(($this->_config['quality']-1)/10)); break;
    ...
    private function image_outline($img){
        ...
        // 获取图片宽高
        $img_w = imagesx($img);
        $img_h = imagesy($img);        // 计算描边尺寸，没有设定则按比例自动计算
        $bg_w = isset($this->_config['logo_outline_size'])? intval($img_w + $this->_config['logo_outline_size']) : $img_w + (int)($img_w/5);
        $bg_h = isset($this->_config['logo_outline_size'])? intval($img_h + $this->_config['logo_outline_size']) : $img_h + (int)($img_h/5);        // 创建底图对象
        $bg_img = imagecreatetruecolor($bg_w, $bg_h);        // 设置底图颜色
        $rgb = $this->hex2rgb($this->_config['logo_outline_color']);
        $bgcolor = imagecolorallocate($bg_img, $rgb['r'], $rgb['g'], $rgb['b']);        // 填充底图颜色
    ...
    private function image_fillet($img){
        ...
        for($x=0; $x<$img_w; $x++){
        for($y=0; $y<$img_h; $y++){
        $rgb = imagecolorat($img, $x, $y);                // 不在图片四角范围，直接画图
